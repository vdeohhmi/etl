...
if not api_key:
    st.warning("Set OPENAI_API_KEY before running!")
    ...
    if key:
        ...
        with st.form("transform_form", clear_on_submit=False):
            if op == "rename":
                ...
                cols = st.multiselect("Columns to one-hot", df.select_dtypes("object").columns)
        ...
        # **Always** apply steps and show live preview
        out = apply_steps(df)
        st.session_state.datasets[key] = out
        st.write("**Transformed Preview:**")
        st.data_editor(out, use_container_width=True)
...
with tabs[3]:
    ...
    if key:
        ...
        if not nums.empty:
            st.plotly_chart(px.imshow(nums.corr(), text_auto=True),
                            use_container_width=True)
...
with tabs[7]:
    ...
    if not key:
        ...
        if tool == "Compute Column":
            newc = st.text_input("New column name")
            desc = st.text_area("Describe logic (plain English)")
            ...
            # Alt manual entry
            alt = st.text_input("Or paste your own formula here")
            if st.button("Apply Manual Formula"):
                if alt.strip():
                    st.session_state.steps.append({
                        "type":"compute","new":newc,"expr":alt.strip(),"desc":"manual"
                    })
            ...
            if st.button("Run Query"):
                ...
                prompt = (
                    ...
                    f"Question: {q}. Provide a concise markdown answer."
                )
                with st.spinner("AI…"):
                    resp = client.chat.completions.create(
                        model="gpt-4o-mini",
            ...
            if mode == "Single Column":
                ...
                if st.button("Generate Story"):
                    ...
                    prompt = (
                        ...
                        f"Analyze column '{col}': distribution, missing, outliers."
                    )
                    with st.spinner("AI…"):
                        resp = client.chat.completions.create(
                            model="gpt-4o-mini",
                ...
                if st.button("Generate Dataset Story"):
                    ...
                    prompt = (
                        f"You are a data journalist. Columns: {cols}. Sample: {sample}. "
                        "Write a detailed dataset report."
                    )
                    with st.spinner("AI…"):
                        resp = client.chat.completions.create(
                            model="gpt-4o-mini",
...
with tabs[8]:
    ...
    if key:
        ...
        tgt = st.selectbox("Target column", df.columns)
        wt_opt = [None] + list(df.columns)
        wt = st.selectbox("Weight column (optional)", wt_opt)
        if st.button("Generate Graph"):
            ...
            top5 = set(e[:2] for e in sorted(G.edges(data=True),
