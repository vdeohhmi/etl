...
def clean_expr(raw: str) -> str:
    """
    Remove wrapping ``` fences and stray backticks.
    """
    ...
    if key:
        ...
        with st.form('transform_form'):
            if op=='rename':
                ...
                if st.form_submit_button('Add'): st.session_state.steps.append({'type':'rename','old':old,'new':new,'desc':f'{old}→{new}'})
            elif op=='filter':
                expr=st.text_input('Expr')
                if st.form_submit_button('Add'): st.session_state.steps.append({'type':'filter','expr':expr,'desc':expr})
            elif op=='compute':
                newc=st.text_input('New col'); desc=st.text_area('Logic')
                if st.form_submit_button('AI Add'):
                    prompt=f"Generate expr for {newc}: {desc}"
                ...
                if st.form_submit_button('Add'): st.session_state.steps.append({'type':'drop_const','desc':'drop const'})
            elif op=='onehot':
                cols=st.multiselect('Cols', df.select_dtypes('object').columns)
                if st.form_submit_button('Add'): st.session_state.steps.append({'type':'onehot','cols':cols,'desc':','.join(cols)})
            elif op=='join':
                ...
                if st.form_submit_button('Add'): st.session_state.steps.append({'type':'join','aux':aux,'left':left,'right':right,'how':how,'desc':f'{aux} on {left}={right}'})
            else:
                if st.form_submit_button('Add'): st.session_state.steps.append({'type':'impute','desc':'impute'})
        updated=apply_steps(df)
        st.session_state.datasets[key]=updated
        st.write('**Preview:**')
        st.data_editor(updated, key=f'preview_{key}_{len(st.session_state.versions)}', use_container_width=True)
...
with tabs[3]:
    ...
    if key:
        ...
        if not num.empty: st.plotly_chart(px.imshow(num.corr(), text_auto=True), use_container_width=True)
...
with tabs[4]:
    ...
    if key:
        ...
        if st.button('Export'):
            ...
            elif fmt=='Excel': buf=BytesIO();df.to_excel(buf,index=False,engine='openpyxl'); st.download_button('Excel',buf.getvalue(),'data.xlsx')
            else: tbl=st.text_input('Table');
                if st.button('Write'): write_pandas(snowflake.connector.connect(
                    ...
                    ),df,tbl); st.success(f'Wrote to {tbl}')
...
with tabs[5]:
    ...
    if key and st.session_state.versions:
        for i,snap in enumerate(st.session_state.versions,1): c1,c2=st.columns([0.7,0.3]);c1.write(f'{i}. snap');
            if c2.button('Revert', key=f'hist_{i}'): st.session_state.datasets[key]=snap
...
with tabs[7]:
    ...
    key=st.session_state.current
    if not key: st.info('Select dataset')
    else:
        df=st.session_state.datasets[key]
        st.data_editor(df, key=f'ai_prev_{key}_{len(st.session_state.steps)}', use_container_width=True)
        tool=st.selectbox('Tool',['Compute Column','NL Query','Story'])
        if tool=='Compute Column':
            newc=st.text_input('New col', key='ai_newc'); desc=st.text_area('Logic', key='ai_desc')
            ...
            if st.button('Apply Alt'): st.session_state.steps.append({'type':'compute','new':newc,'expr':alt,'desc':'manual'})
            df2=apply_steps(df); st.session_state.datasets[key]=df2; st.data_editor(df2, key=f'ai_upd_{key}_{len(st.session_state.steps)}', use_container_width=True)
        elif tool=='NL Query':
            ...
            if mode=='Single Column':
                ...
                if st.button('Gen DS Story'): resp=client.chat.completions.create(model='gpt-4o-mini', messages=[{'role':'user','content':'Report the dataset.'}]); st.markdown(resp.choices[0].message.content)
...
with tabs[8]:
    ...
    if key:
        ...
        if st.button('Gen Graph'):
            ...
            for u,v,d in G.edges(data=True): net.add_edge(u,v,value=d['weight'], width=4 if (u,v) in top5 or (v,u) in top5 else 1, color='red' if (u,v) in top5 or (v,u) in top5 else 'rgba(200,200,200,0.2)', title=f"W:{d['weight']}")
